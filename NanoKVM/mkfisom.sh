#!/bin/sh
# mkfisom.sh - make an empty, formatted disk image of a set size, or mount one
# (revision 8)
#
...
usage() {
    cat <<HELP
mkfisom - make an empty, formatted disk image (FAT32 or exFAT), or mount one

Usage: ${0##*/} [-s SIZE] [-l LABEL] [-o NAME]
       ${0##*/} -m FILE
  ...
  -o NAME   output file name                          (default: $STAMP)
            ...
            unit, e.g. -s 4096m -o disc makes disc-4g-fat32.iso.
            A NAME ending in .iso is used as it is, with nothing added.
            If NAME is a folder, the image gets the default name in it.
  -m FILE   mount an image on a folder in the current directory, named
            after FILE without its extension. Run it again with the same
...
An image mounted by the NanoKVM is never formatted or mounted here.

Example: ${0##*/} -s 2g -l BACKUP -o disc   (makes disc-2g-fat32.iso)
         ${0##*/} -m disc.iso     (mounts on ./disc, run again to unmount)
HELP
    exit 0
}
...
# folder gets the default name in it, and a NAME ending in .iso is used as is
n=$((num << bits)) unit=
for u in k m g t; do
    [ $((n % 1024)) -eq 0 ] || break
    n=$((n >> 10)) unit=$u
